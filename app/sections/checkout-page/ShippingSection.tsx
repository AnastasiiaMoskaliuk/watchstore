...
const ShippingSection: FC<{
  ...
  useEffect(() => {
    let { department, street, house, flat, postomat } = form.values;
    ...
    if (selectedOption === "Department" && department != "") {
      ...
      setShippingValue({ department, selectedOption });
    } else if (
      ...
      setShippingValue({ street, house, flat, selectedOption });
    } else if (selectedOption === "Postomat" && postomat != "") {
      localStorage.setItem(
        "shippingInfo",
        JSON.stringify({ postomat, selectedOption })
      );
      postomat = postomat.replace('Поштомат "Нова Пошта" ', "");
      setShippingValue({ postomat, selectedOption });
    }
  ...
  const handleSelect = async (value: string) => {
    switch (selectedOption) {
      case "Courier":
        if (!settlementRef) {
          setStreets([]);
          setValidationErrors("Оберіть населений пункт!");
          return;
        }

        if (!value.trim()) {
          const defaultStreet = await getStreets(
            "ко",
            settlementRef,
            setInfoMessage
          );
          setStreets(defaultStreet);
        ...
        if (selectedStreet) {
          ...
          const street = await getStreets(value, settlementRef, setInfoMessage);
          if (street.length === 0) {
      ...
      case "Postomat":
        ...
        if (!value.trim()) {
          const defaultPostomat = await getPostomates(
            "пош",
            cityRef,
            setInfoMessage
          );
          setPostomates(defaultPostomat);
          setValidationErrors(null);
          return;
        }
        ...
        const postomat = await getPostomates(value, cityRef, setInfoMessage);
      ...
      case "Department":
        ...
        if (!value.trim()) {
          const defaultDepartment = await getDepartments(
            "від",
            cityRef,
            setInfoMessage
          );
  ...
  const handleContinue = () => {
    ...
    if (
      validationErrors == "Не знайдено!" ||
      validationErrors == "Оберіть населений пункт!" ||
      validationError
    ) {
      onContinue(false);
      return;
  ...
  return (
    <>
      ...
      >
        ...
        >
          {selectedOption === "Department" && (
            <>
              ...
              {validationErrors && (
                <p className="text-darkBlack text-[14px] m-auto">
                  {validationErrors}
                </p>
              )}
            </>
        ...
        >
          {selectedOption === "Courier" && (
            <>
              ...
              {validationErrors && (
                <p className="text-darkBlack text-[14px] m-auto mt-[20px]">
                  {validationErrors}
                </p>
              )}
            </>
        ...
        {error && (
          <p className="text-[14px] text-darkBlack text-center">{error}</p>
        )}
