...
const BasicInfoSection: FC<{
  ...
  useEffect(() => {
    if (form.values.city == "") {
      if (form.values.cityRef != "" || form.values.settlementRef != "") {
        form.setFieldValue("cityRef", "");
        form.setFieldValue("settlementRef", "");
        setCityRef("");
        setSettlementRef("");
      }
    }
  ...
  const handleCitySelect = async (value: string) => {
    if (!value.trim()) {
      const defaultCities = await getCities("к", setInfoMessage);
      setCities(defaultCities.slice(0, 10));
      setError(null);
      return;
    }
        ...
        {isStart && (
          <>
            <Input
              ...
              scrollable
            />
            {error && <p className="text-darkBlack text-[14px]">{error}</p>}
          </>
        )}
