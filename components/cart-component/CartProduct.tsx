    ...
    <li className="text-black pb-[15px] border-b border-[#ECEDF1] flex gap-[15px] items-center">
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex items-center justify-between">
          <h4 className="text-silver text-[14px]">{card.title}</h4>
          <button onClick={(e) => onHandleClickDelete(e, card.id)}>
            <Image
              src={Delete}
              alt="delete button"
              className="object-fit h-6 w-6 hover:scale-110 duration-300"
            />
          </button>
        </div>

        <span className="text-black text-[20px] mb-[15px]">{card.price} грн</span>
        ...
        <div className="flex items-center border-2 rounded-md w-fit overflow-hidden">
          <button
            ...
            className={`w-8 h-8 rounded-sm border-r-2 
              ${
                card.quantity > 1 ? "hover:bg-darkBlack hover:text-snow bg-snow" : "bg-gray-200 bg-opacity-80 cursor-not-allowed text-[gray]"
              }`}
          >
            -
          </button>
          ...
          disabled={card.quantity >= card.maxQuantity}
            ...
            className={`w-8 h-8 rounded-sm border-l-2  ${
              card.quantity < card.maxQuantity
                ? "hover:bg-darkBlack hover:text-snow bg-snow" : "bg-gray-200 bg-opacity-80 cursor-not-allowed text-[gray]"
            }`}
          >
            +
          </button>
