    ...
    if (inputType === "select") {
      ...
      return (
        <div className={`${className} flex flex-col w-full mini:w-[320px]`}>
          <div className={` relative`} ref={selectRef}>
            <input
              className={`${borderClass} py-[16px] px-[30px] pr-[50px] rounded-[5px] w-full focus:outline-none focus:border-darkBlack`}
              type={type}
            ...
            {isOpen && options.length > 0 && (
              ...
              >
                {filteredOptions.map((option) => (
                  <li
                    key={option.value}
                    className="p-2 hover:bg-gray-200 cursor-pointer text-gray-700 rounded-lg !text-left"
          ...
          {error && (
            <p
              className={`text-[14px] ${
                errorType === "critical" ? "text-darkBlack" : "text-gray-400"
              }`}
            >
              {error}
            </p>
      ...
      return (
        <div className={`${className} flex flex-col w-full`}>
          <input
            className={`${borderClass} ${widthClass} ${textClass} py-[16px] px-[30px] rounded-[5px] focus:outline-none focus:border-[1px] focus:border-darkBlack`}
            type={type}
            placeholder={placeholder}
            pattern={pattern}
            value={value}
          ...
          {error && (
            <p
              className={`text-[14px] ${
                errorType === "critical" ? "text-darkBlack" : "text-snow"
              }`}
            >
              {error}
            </p>
      ...
      return (
        <div className={`${className} flex flex-col w-full`}>
          <div className="relative">
            <input
              className={`${borderClass} ${widthClass} ${textClass} relative py-[16px] px-[30px] rounded-[5px] focus:outline-none focus:border-[1px] focus:border-darkBlack`}
      ...
      return (
        <>
          <textarea
            ...
            disabled={disabled}
            onChange={onChange}
            className={`${className} ${borderClass} ${textClass} resize-none py-[16px] px-[30px] w-full h-[160px] rounded border-[1px] rounded-[5px] border-[#EAECF5] lg:w-[90%] xl:w-[70%] focus:outline-none focus:border-[1px] focus:border-darkBlack`}
          />
          {error && (
            <p
              className={`text-[14px] ${
